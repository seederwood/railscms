ActiveAdmin.register Block do
  ...
  form do |f|
    f.inputs 'Details' do
      ...
      f.input :position, label: 'Position', as: :select, collection: [%w[Jumbotron jumbotron], %w[Block block]]
      f.input :display, label: 'Display', as: :select, collection: [['All Pages', 'all'], ['Home Page Only', 'home'], ['All But Home Page', 'nohome']]
